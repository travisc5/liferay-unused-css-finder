...
var configFilePath = path.join(userhome(), '.lucf.json');

var CONFIG_VERSION = "1";

function Instance(username, password, mailDomain, host, port, cssFile) {
	this.username = username || 'test';
	this.password = password || 'test';
	this.mailDomain = mailDomain || 'liferay.com';
	this.host = host || 'localhost';
	this.port = port || '8080';
	this.cssFile = cssFile || 'custom.css';
...
module.exports.getCurrentCssFile = function() {
	var currentCssFile = currentInstanceConfig.cssFile;
...
module.exports.setInstanceConfig = function(name, username, password, mailDomain, host, port, cssFile) {
	var newInstance = new Instance(username, password, mailDomain, host, port, cssFile);
